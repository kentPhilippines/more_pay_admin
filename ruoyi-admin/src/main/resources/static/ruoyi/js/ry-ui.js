...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            statisticsUrlDeal: function (title, width, height) {
                table.set();
                var _url = url = table.options.statisticsUrlDeal;
                var _width = $.common.isEmpty(width) ? "800" : width;
                var _height = $.common.isEmpty(height) ? ($(window).height() - 50) : height;
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios|ipad)/i)) {
            ...
            // 当天商户数据统计
            statisticsMerchant: function (title, width, height) {
                table.set();
                var _url = url = table.options.statisticsMerchantUrl;
                var _width = $.common.isEmpty(width) ? "800" : width;
                var _height = $.common.isEmpty(height) ? ($(window).height() - 50) : height;
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios|ipad)/i)) {
